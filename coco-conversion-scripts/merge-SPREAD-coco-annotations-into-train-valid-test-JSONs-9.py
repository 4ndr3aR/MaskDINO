...
def split_dataset(coco_data, train_ratio=0.7, valid_ratio=0.2):
    ...
    # Reassign unique IDs for images and annotations
    def reassign_ids(images, annotations):
        # Create new mappings for image and annotation IDs
        new_image_id_map = {old_id: new_id for new_id, old_id in enumerate(set(img['id'] for img in images), 1)}
        new_ann_id_map = {}
        
        # Update image IDs
        for img in images:
            img['id'] = new_image_id_map[img['id']]
        
        # Update annotation IDs and image_ids
        new_annotations = []
        for ann_id, ann in enumerate(annotations, 1):
            ann = ann.copy()  # Create a copy to avoid modifying the original
            ann['id'] = ann_id
            ann['image_id'] = new_image_id_map[ann['image_id']]
            new_annotations.append(ann)
        
        return images, new_annotations

    train_images, train_annotations = reassign_ids(train_images, train_annotations)
    valid_images, valid_annotations = reassign_ids(valid_images, valid_annotations)
    test_images, test_annotations = reassign_ids(test_images, test_annotations)

    # Return the split datasets with the original categories and unique IDs
    return (
...
def main(base_dir, output_dir):
    ...
    save_coco_annotations(train_set, os.path.join(output_dir, 'train-9.json'))
    save_coco_annotations(valid_set, os.path.join(output_dir, 'valid-9.json'))
    save_coco_annotations(test_set , os.path.join(output_dir, 'test-9.json'))
